...
printf '\n── where the hook gets installed ──\n\n'

# A forge owns hooks/pre-receive. Gitea and GitLab generate one there for branch
# protection, push options and LFS, and load third-party hooks from
# pre-receive.d beside it. Writing over the generated file removes forge
# behaviour without any push failing, so nothing announces the loss.
INSTALLER="$(cd "$HERE/.." && pwd)/install-pre-receive.sh"

plain_repo="$WORKDIR/plain.git"
git init -q --bare "$plain_repo"
if "$INSTALLER" --local "$plain_repo" >/dev/null 2>&1 \
   && [[ -x "$plain_repo/hooks/pre-receive" ]] \
   && [[ -f "$plain_repo/hooks/pre-receive.conf" ]]; then
  report PASS plain-repo-hook-direct "hooks/pre-receive"
else
  report FAIL plain-repo-hook-direct "did not install into a plain bare repo"
fi

forge_repo="$WORKDIR/forge.git"
git init -q --bare "$forge_repo"
mkdir -p "$forge_repo/hooks/pre-receive.d"
printf '#!/bin/sh\n# AUTO GENERATED BY GITEA, DO NOT MODIFY\nexit 0\n' \
  > "$forge_repo/hooks/pre-receive"
chmod +x "$forge_repo/hooks/pre-receive"
generated_before="$(cat "$forge_repo/hooks/pre-receive")"

if "$INSTALLER" --local "$forge_repo" >/dev/null 2>&1 \
   && [[ -x "$forge_repo/hooks/pre-receive.d/sdlc-destructive-guard" ]]; then
  report PASS forge-layout-uses-directory "installed into pre-receive.d"
else
  report FAIL forge-layout-uses-directory "did not use the forge hook directory"
fi

if [[ "$(cat "$forge_repo/hooks/pre-receive")" == "$generated_before" ]]; then
  report PASS forge-hook-untouched "left the forge's own hook alone"
else
  report FAIL forge-hook-untouched "overwrote the generated hook"
fi

if [[ -f "$forge_repo/hooks/pre-receive.d/pre-receive.conf" ]]; then
  report PASS conf-beside-the-hook "the hook reads it from its own directory"
else
  report FAIL conf-beside-the-hook "config is where the hook cannot find it"
fi

nodir_repo="$WORKDIR/forge-nodir.git"
git init -q --bare "$nodir_repo"
printf '#!/bin/sh\n# AUTO GENERATED BY GITEA, DO NOT MODIFY\nexit 0\n' \
  > "$nodir_repo/hooks/pre-receive"
chmod +x "$nodir_repo/hooks/pre-receive"
before="$(cat "$nodir_repo/hooks/pre-receive")"
out="$("$INSTALLER" --local "$nodir_repo" 2>&1 || true)"
if [[ "$(cat "$nodir_repo/hooks/pre-receive")" == "$before" ]]; then
  report PASS generated-hook-not-clobbered "refused, and changed nothing"
else
  report FAIL generated-hook-not-clobbered "overwrote a forge-generated hook"
fi
if printf '%s' "$out" | grep -q "pre-receive.d"; then
  report PASS refusal-explains-fix "told the operator to create the directory"
else
  report FAIL refusal-explains-fix "refused without saying how to proceed"
fi
